...
\noindent
\ttfamily
\hlstd{}\hlslc{//\ convert\ an\ R\ matrix\ to\ a\ GSL\ matrix}\hspace*{\fill}\\
\hlstd{}\hlkwd{matrix}\hlstd{}\hlsym{(\ }\hlstd{SEXP\ x}\hlsym{)\ }\hlstd{}\hlkwa{throw}\hlstd{}\hlsym{(::}\hlstd{Rcpp}\hlsym{::}\hlstd{not\textunderscore compatible}\hlsym{)}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ encapsulate\ a\ GSL\ matrix\ pointer}\hspace*{\fill}\\
\hlstd{}\hlkwd{matrix}\hlstd{}\hlsym{(\ }\hlstd{gsl\textunderscore matrix}\hlsym{{*}\ }\hlstd{x}\hlsym{)}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ create\ a\ new\ matrix\ with\ the\ given\ number\ of\ rows\ and\ columns}\hspace*{\fill}\\
\hlstd{}\hlkwd{matrix}\hlstd{}\hlsym{(\ }\hlstd{}\hlkwb{int\ }\hlstd{nrow}\hlsym{,\ }\hlstd{}\hlkwb{int\ }\hlstd{ncol}\hlsym{)}\hlstd{}\hspace*{\fill}\\
\mbox{}
\normalfont
\end{Hchunk}\vspace{1em}
\end{small}
...
\noindent
\ttfamily
\hlstd{gsltype}\hlsym{{*}\ }\hlstd{data\ }\hlsym{;}\hspace*{\fill}\\
\hlstd{}\hlkwc{operator\ }\hlstd{gsltype}\hlsym{{*}()\usebox{\hlsmallboxopenbrace}\ }\hlstd{}\hlkwa{return\ }\hlstd{data\ }\hlsym{;\ \usebox{\hlsmallboxclosebrace}}\hspace*{\fill}\\
\hlstd{gsltype}\hlsym{{*}\ }\hlstd{}\hlkwc{operator}\hlstd{}\hlsym{{-}\usebox{\hlsmallboxgreaterthan}()\ }\hlstd{}\hlkwb{const\ }\hlstd{}\hlsym{\usebox{\hlsmallboxopenbrace}\ }\hlstd{}\hlkwa{return\ }\hlstd{data}\hlsym{;\ \usebox{\hlsmallboxclosebrace}}\hspace*{\fill}\\
\hlstd{gsltype}\hlsym{\&\ }\hlstd{}\hlkwc{operator}\hlstd{}\hlsym{{*}()\ }\hlstd{}\hlkwb{const\ }\hlstd{}\hlsym{\usebox{\hlsmallboxopenbrace}\ }\hlstd{}\hlkwa{return\ }\hlstd{}\hlsym{{*}}\hlstd{data}\hlsym{;\ \usebox{\hlsmallboxclosebrace}}\hlstd{}\hspace*{\fill}\\
\mbox{}
\normalfont
\end{Hchunk}\vspace{1em}
\end{small}
...
\noindent
\ttfamily
\hlstd{RcppGSL}\hlsym{::}\hlstd{matrix}\hlsym{\usebox{\hlsmallboxlessthan}}\hlstd{}\hlkwb{int}\hlstd{}\hlsym{\usebox{\hlsmallboxgreaterthan}\ }\hlstd{}\hlkwd{mat}\hlstd{}\hlsym{(}\hlstd{}\hlnum{10}\hlstd{}\hlsym{,}\hlstd{}\hlnum{10}\hlstd{}\hlsym{);}\hlstd{\ \ \ \ \ \ \ \ }\hlsym{}\hlstd{}\hlslc{//\ create\ a\ matrix\ of\ size\ 10x10}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlkwa{for}\hlstd{}\hlsym{(\ }\hlstd{}\hlkwb{int\ }\hlstd{i}\hlsym{=}\hlstd{}\hlnum{0}\hlstd{}\hlsym{;\ }\hlstd{i}\hlsym{\usebox{\hlsmallboxlessthan}}\hlstd{}\hlnum{10}\hlstd{}\hlsym{:\ }\hlstd{i}\hlsym{++)\ \usebox{\hlsmallboxopenbrace}}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ \ \ }\hlsym{}\hlstd{}\hlslc{//\ fill\ the\ diagonal}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ }\hlstd{}\hlkwd{mat}\hlstd{}\hlsym{(}\hlstd{i}\hlsym{,}\hlstd{i}\hlsym{)\ =\ }\hlstd{i\ }\hlsym{;}\hspace*{\fill}\\
\hlstd{}\hlsym{\usebox{\hlsmallboxclosebrace}}\hlstd{}\hspace*{\fill}\\
\mbox{}
\normalfont
\end{Hchunk}\vspace{1em}
...
\noindent
\ttfamily
\hlstd{AC\textunderscore INIT}\hlsym{({[}}\hlstd{RcppGSLExample}\hlsym{{]},\ }\hlstd{}\hlnum{0.1.0}\hlstd{}\hlsym{)}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{\#\#\ Use\ gsl{-}config\ to\ find\ arguments\ for\ compiler\ and\ linker\ flags}\hspace*{\fill}\\
\hlstd{}\hlslc{\#\#}\hspace*{\fill}\\
\hlstd{}\hlslc{\#\#\ Check\ for\ non{-}standard\ programs:\ gsl{-}config(1)}\hspace*{\fill}\\
\hlstd{AC\textunderscore PATH\textunderscore PROG}\hlsym{({[}}\hlstd{GSL\textunderscore CONFIG}\hlsym{{]},\ {[}}\hlstd{gsl}\hlsym{{-}}\hlstd{config}\hlsym{{]})}\hspace*{\fill}\\
\hlstd{}\hlslc{\#\#\ If\ gsl{-}config\ was\ found,\ let's\ use\ it}\hspace*{\fill}\\
\hlstd{}\hlkwa{if\ }\hlstd{}\hlkwb{test\ }\hlstd{}\hlstr{"\$\usebox{\hlsmallboxopenbrace}GSL\textunderscore CONFIG\usebox{\hlsmallboxclosebrace}"}\hlstd{\ }\hlsym{!=\ }\hlstd{}\hlstr{""}\hlstd{}\hlsym{;\ }\hlstd{}\hlkwa{then}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ }\hlstd{}\hlslc{\#\ Use\ gsl{-}config\ for\ header\ and\ linker\ arguments\ (without\ BLAS\ which\ we\ get\ from\ R)}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ }\hlstd{GSL\textunderscore CFLAGS}\hlsym{=}\hlstd{}\hlstr{`\$\usebox{\hlsmallboxopenbrace}GSL\textunderscore CONFIG\usebox{\hlsmallboxclosebrace}\ {-}{-}cflags`}\hlstd{\hspace*{\fill}\\
}\hlstd{\ \ \ \ }\hlstd{GSL\textunderscore LIBS}\hlsym{=}\hlstd{}\hlstr{`\$\usebox{\hlsmallboxopenbrace}GSL\textunderscore CONFIG\usebox{\hlsmallboxclosebrace}\ {-}{-}libs{-}without{-}cblas`}\hlstd{}\hspace*{\fill}\\
\hlkwa{else}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ }\hlstd{AC\textunderscore MSG\textunderscore ERROR}\hlsym{({[}}\hlstd{gsl}\hlsym{{-}}\hlstd{config\ not\ found}\hlsym{,\ }\hlstd{is\ GSL\ installed?}\hlsym{{]})}\hspace*{\fill}\\
\hlstd{}\hlkwa{fi}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{\#\#\ Use\ Rscript\ to\ query\ Rcpp\ for\ compiler\ and\ linker\ flags}\hspace*{\fill}\\
\hlstd{}\hlslc{\#\#\ link\ flag\ providing\ libary\ as\ well\ as\ path\ to\ library,\ and\ optionally\ rpath}\hspace*{\fill}\\
\hlstd{RCPP\textunderscore LDFLAGS}\hlsym{=}\hlstd{}\hlstr{`\$\usebox{\hlsmallboxopenbrace}R\textunderscore HOME\usebox{\hlsmallboxclosebrace}/bin/Rscript\ {-}e\ 'Rcpp:::LdFlags()'`}\hlstd{}\hspace*{\fill}\\
\hspace*{\fill}\\
\hlslc{\#\ Now\ substitute\ these\ variables\ in\ src/Makevars.in\ to\ create\ src/Makevars}\hspace*{\fill}\\
\hlstd{AC\textunderscore SUBST}\hlsym{(}\hlstd{GSL\textunderscore CFLAGS}\hlsym{)}\hspace*{\fill}\\
\hlstd{AC\textunderscore SUBST}\hlsym{(}\hlstd{GSL\textunderscore LIBS}\hlsym{)}\hspace*{\fill}\\
\hlstd{AC\textunderscore SUBST}\hlsym{(}\hlstd{RCPP\textunderscore LDFLAGS}\hlsym{)}\hspace*{\fill}\\
\hlstd{\hspace*{\fill}\\
AC\textunderscore OUTPUT}\hlsym{(}\hlstd{src}\hlsym{/}\hlstd{Makevars}\hlsym{)}\hlstd{}\hspace*{\fill}\\
\mbox{}
\normalfont
\end{Hchunk}\vspace{1em}
\end{small}
...
\noindent
\ttfamily
\hlstd{}\hlslc{\#!/bin/sh}\hspace*{\fill}\\
\hlstd{\hspace*{\fill}\\
GSL\textunderscore CFLAGS}\hlsym{=}\hlstd{}\hlstr{`\$\usebox{\hlsmallboxopenbrace}R\textunderscore HOME\usebox{\hlsmallboxclosebrace}/bin/Rscript\ {-}e\ "RcppGSL:::CFlags()"`}\hlstd{\hspace*{\fill}\\
GSL\textunderscore LIBS}\hlsym{=}\hlstd{}\hlstr{`\$\usebox{\hlsmallboxopenbrace}R\textunderscore HOME\usebox{\hlsmallboxclosebrace}/bin/Rscript\ {-}e\ "RcppGSL:::LdFlags()"`}\hlstd{\hspace*{\fill}\\
RCPP\textunderscore LDFLAGS}\hlsym{=}\hlstd{}\hlstr{`\$\usebox{\hlsmallboxopenbrace}R\textunderscore HOME\usebox{\hlsmallboxclosebrace}/bin/Rscript\ {-}e\ "Rcpp:::LdFlags()"`}\hlstd{}\hspace*{\fill}\\
\hspace*{\fill}\\
\hlkwc{sed\ }\hlstd{}\hlsym{{-}}\hlstd{e\ }\hlstr{"s\textbar @GSL\textunderscore LIBS@\textbar \$\usebox{\hlsmallboxopenbrace}GSL\textunderscore LIBS\usebox{\hlsmallboxclosebrace}\textbar "}\hlstd{\ $\backslash$\hspace*{\fill}\\
}\hlstd{\ \ \ \ }\hlstd{}\hlsym{{-}}\hlstd{e\ }\hlstr{"s\textbar @GSL\textunderscore CFLAGS@\textbar \$\usebox{\hlsmallboxopenbrace}GSL\textunderscore CFLAGS\usebox{\hlsmallboxclosebrace}\textbar "}\hlstd{\ $\backslash$\hspace*{\fill}\\
}\hlstd{\ \ \ \ }\hlstd{}\hlsym{{-}}\hlstd{e\ }\hlstr{"s\textbar @RCPP\textunderscore LDFLAGS@\textbar \$\usebox{\hlsmallboxopenbrace}RCPP\textunderscore LDFLAGS\usebox{\hlsmallboxclosebrace}\textbar "}\hlstd{\ $\backslash$\hspace*{\fill}\\
}\hlstd{\ \ \ \ }\hlstd{src}\hlsym{/}\hlstd{Makevars.}\hlkwa{in\ }\hlstd{}\hlsym{\usebox{\hlsmallboxgreaterthan}\ }\hlstd{src}\hlsym{/}\hlstd{Makevars}\hspace*{\fill}\\
\mbox{}
\normalfont
\end{Hchunk}\vspace{1em}
\end{small}
...
\noindent
\ttfamily
\hlstd{GSL\textunderscore CFLAGS}\hlsym{=}\hlstd{}\hlstr{`\$\usebox{\hlsmallboxopenbrace}R\textunderscore HOME\usebox{\hlsmallboxclosebrace}/bin\$\usebox{\hlsmallboxopenbrace}R\textunderscore ARCH\textunderscore BIN\usebox{\hlsmallboxclosebrace}/Rscript.exe\ {-}e\ "RcppGSL:::CFlags()"`}\hlstd{\hspace*{\fill}\\
GSL\textunderscore LIBS}\hlsym{=}\hlstd{}\hlstr{`\$\usebox{\hlsmallboxopenbrace}R\textunderscore HOME\usebox{\hlsmallboxclosebrace}/bin\$\usebox{\hlsmallboxopenbrace}R\textunderscore ARCH\textunderscore BIN\usebox{\hlsmallboxclosebrace}/Rscript.exe\ {-}e\ "RcppGSL:::LdFlags()"`}\hlstd{\hspace*{\fill}\\
RCPP\textunderscore LDFLAGS}\hlsym{=}\hlstd{}\hlstr{`\$\usebox{\hlsmallboxopenbrace}R\textunderscore HOME\usebox{\hlsmallboxclosebrace}/bin\$\usebox{\hlsmallboxopenbrace}R\textunderscore ARCH\textunderscore BIN\usebox{\hlsmallboxclosebrace}/Rscript.exe\ {-}e\ "Rcpp:::LdFlags()"`}\hlstd{}\hspace*{\fill}\\
\hspace*{\fill}\\
\hlkwc{sed\ }\hlstd{}\hlsym{{-}}\hlstd{e\ }\hlstr{"s\textbar @GSL\textunderscore LIBS@\textbar \$\usebox{\hlsmallboxopenbrace}GSL\textunderscore LIBS\usebox{\hlsmallboxclosebrace}\textbar "}\hlstd{\ $\backslash$\hspace*{\fill}\\
}\hlstd{\ \ \ \ }\hlstd{}\hlsym{{-}}\hlstd{e\ }\hlstr{"s\textbar @GSL\textunderscore CFLAGS@\textbar \$\usebox{\hlsmallboxopenbrace}GSL\textunderscore CFLAGS\usebox{\hlsmallboxclosebrace}\textbar "}\hlstd{\ $\backslash$\hspace*{\fill}\\
}\hlstd{\ \ \ \ }\hlstd{}\hlsym{{-}}\hlstd{e\ }\hlstr{"s\textbar @RCPP\textunderscore LDFLAGS@\textbar \$\usebox{\hlsmallboxopenbrace}RCPP\textunderscore LDFLAGS\usebox{\hlsmallboxclosebrace}\textbar "}\hlstd{\ $\backslash$\hspace*{\fill}\\
}\hlstd{\ \ \ \ }\hlstd{src}\hlsym{/}\hlstd{Makevars.}\hlkwa{in\ }\hlstd{}\hlsym{\usebox{\hlsmallboxgreaterthan}\ }\hlstd{src}\hlsym{/}\hlstd{Makevars.win}\hspace*{\fill}\\
\mbox{}
\normalfont
\end{Hchunk}\vspace{1em}
\end{small}
...
\ttfamily
\hlstd{}\hldir{\#include\ \usebox{\hlsmallboxlessthan}RcppGSL.h\usebox{\hlsmallboxgreaterthan}}\hspace*{\fill}\\
\hlstd{}\hldir{\#include\ \usebox{\hlsmallboxlessthan}gsl/gsl\textunderscore matrix.h\usebox{\hlsmallboxgreaterthan}}\hspace*{\fill}\\
\hlstd{}\hldir{\#include\ \usebox{\hlsmallboxlessthan}gsl/gsl\textunderscore blas.h\usebox{\hlsmallboxgreaterthan}}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlkwc{extern\ }\hlstd{}\hlstr{"C"}\hlstd{\ SEXP\ }\hlkwd{colNorm}\hlstd{}\hlsym{(}\hlstd{SEXP\ sM}\hlsym{)\ \usebox{\hlsmallboxopenbrace}}\hspace*{\fill}\\
\hlstd{\hspace*{\fill}\\
}\hlstd{\ \ }\hlstd{}\hlkwa{try\ }\hlstd{}\hlsym{\usebox{\hlsmallboxopenbrace}}\hspace*{\fill}\\
\hlstd{\hspace*{\fill}\\
}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{RcppGSL}\hlsym{::}\hlstd{matrix}\hlsym{\usebox{\hlsmallboxlessthan}}\hlstd{}\hlkwb{double}\hlstd{}\hlsym{\usebox{\hlsmallboxgreaterthan}\ }\hlstd{M\ }\hlsym{=\ }\hlstd{sM}\hlsym{;}\hlstd{\ \ \ \ \ }\hlsym{}\hlstd{}\hlslc{//\ create\ gsl\ data\ structures\ from\ SEXP}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{}\hlkwb{int\ }\hlstd{k\ }\hlsym{=\ }\hlstd{M}\hlsym{.}\hlstd{}\hlkwd{ncol}\hlstd{}\hlsym{();}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{Rcpp}\hlsym{::}\hlstd{NumericVector\ }\hlkwd{n}\hlstd{}\hlsym{(}\hlstd{k}\hlsym{);}\hlstd{\ \ \ \ \ \ \ \ \ \ \ }\hlsym{}\hlstd{}\hlslc{//\ to\ store\ results}\hspace*{\fill}\\
\hlstd{\hspace*{\fill}\\
}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{}\hlkwa{for\ }\hlstd{}\hlsym{(}\hlstd{}\hlkwb{int\ }\hlstd{j\ }\hlsym{=\ }\hlstd{}\hlnum{0}\hlstd{}\hlsym{;\ }\hlstd{j\ }\hlsym{\usebox{\hlsmallboxlessthan}\ }\hlstd{k}\hlsym{;\ }\hlstd{j}\hlsym{++)\ \usebox{\hlsmallboxopenbrace}}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ }\hlstd{RcppGSL}\hlsym{::}\hlstd{vector\textunderscore view}\hlsym{\usebox{\hlsmallboxlessthan}}\hlstd{}\hlkwb{double}\hlstd{}\hlsym{\usebox{\hlsmallboxgreaterthan}\ }\hlstd{colview\ }\hlsym{=\ }\hlstd{}\hlkwd{gsl\textunderscore matrix\textunderscore column\ }\hlstd{}\hlsym{(}\hlstd{M}\hlsym{,\ }\hlstd{j}\hlsym{);}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ }\hlstd{n}\hlsym{{[}}\hlstd{j}\hlsym{{]}\ =\ }\hlstd{}\hlkwd{gsl\textunderscore blas\textunderscore dnrm2}\hlstd{}\hlsym{(}\hlstd{colview}\hlsym{);}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{}\hlsym{\usebox{\hlsmallboxclosebrace}}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{M}\hlsym{.}\hlstd{}\hlkwd{free}\hlstd{}\hlsym{()\ ;}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{}\hlkwa{return\ }\hlstd{n}\hlsym{;}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }\hlsym{}\hlstd{}\hlslc{//\ return\ vector}\hspace*{\fill}\\
\hlstd{\hspace*{\fill}\\
}\hlstd{\ \ }\hlstd{}\hlsym{\usebox{\hlsmallboxclosebrace}\ }\hlstd{}\hlkwa{catch}\hlstd{}\hlsym{(\ }\hlstd{std}\hlsym{::}\hlstd{exception\ }\hlsym{\&}\hlstd{ex\ }\hlsym{)\ \usebox{\hlsmallboxopenbrace}}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{}\hlkwd{forward\textunderscore exception\textunderscore to\textunderscore r}\hlstd{}\hlsym{(\ }\hlstd{ex\ }\hlsym{);}\hspace*{\fill}\\
\hlstd{\hspace*{\fill}\\
}\hlstd{\ \ }\hlstd{}\hlsym{\usebox{\hlsmallboxclosebrace}\ }\hlstd{}\hlkwa{catch}\hlstd{}\hlsym{(...)\ \usebox{\hlsmallboxopenbrace}}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{}\hlsym{::}\hlstd{}\hlkwd{Rf\textunderscore error}\hlstd{}\hlsym{(\ }\hlstd{}\hlstr{"c++\ exception\ (unknown\ reason)"}\hlstd{\ }\hlsym{);}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{}\hlsym{\usebox{\hlsmallboxclosebrace}}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{}\hlkwa{return\ }\hlstd{R\textunderscore NilValue}\hlsym{;\ }\hlstd{}\hlslc{//\ {-}Wall}\hspace*{\fill}\\
\hlstd{}\hlsym{\usebox{\hlsmallboxclosebrace}}\hlstd{}\hspace*{\fill}\\
...
\ttfamily
\hlstd{}\hspace*{\fill}\\
\hlslc{\#\ set\ by\ configure}\hspace*{\fill}\\
\hlstd{GSL\textunderscore CFLAGS\ }\hlsym{=\ }\hlstd{@GSL\textunderscore CFLAGS@\hspace*{\fill}\\
GSL\textunderscore LIBS}\hlstd{\ \ \ }\hlstd{}\hlsym{=\ }\hlstd{@GSL\textunderscore LIBS@\hspace*{\fill}\\
RCPP\textunderscore LDFLAGS\ }\hlsym{=\ }\hlstd{@RCPP\textunderscore LDFLAGS@}\hspace*{\fill}\\
\hspace*{\fill}\\
\hlslc{\#\ combine\ with\ standard\ arguments\ for\ R}\hspace*{\fill}\\
\hlstd{PKG\textunderscore CPPFLAGS\ }\hlsym{=\ }\hlstd{\$}\hlsym{(}\hlstd{GSL\textunderscore CFLAGS}\hlsym{)}\hspace*{\fill}\\
\hlstd{PKG\textunderscore LIBS\ }\hlsym{=\ }\hlstd{\$}\hlsym{(}\hlstd{GSL\textunderscore LIBS}\hlsym{)\ }\hlstd{\$}\hlsym{(}\hlstd{RCPP\textunderscore LDFLAGS}\hlsym{)}\hlstd{}\hspace*{\fill}\\
\mbox{}
\normalfont
\end{Hchunk}\vspace{1em}
\end{small}
