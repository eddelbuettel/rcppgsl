...
\shortcites{GSL}
\author{Dirk Eddelbuettel\\Debian and R Projects \And Romain Fran\c{c}ois\\\proglang{R} Enthusiasts}
...
\Plainauthor{Dirk Eddelbuettel} %% comma-separated
\Plaintitle{RcppGSL: Easier GSL use from R via Rcpp}
\Shorttitle{\pkg{RcppGSL}: Easier \pkg{GSL} use from \proglang{R}} %% a short title (if necessary)
...
\Abstract{
  ...
  \citep{JSS:Rcpp,Eddelbuettel:2013:Rcpp,CRAN:Rcpp} which 
  eases the interface between \proglang{R} and C++.
}
\Keywords{\proglang{R}, \pkg{Rcpp}, \pkg{GSL}}
\Plainkeywords{R, Rcpp, GSL} %% without formatting
...
\definecolor{hlBg}{rgb}{0.97,0.97,0.97}  % background
\definecolor{hlBd}{rgb}{0.9,0.9,0.9}     % border
\renewenvironment{Hchunk}{\vspace{0.125em}\noindent\begin{lrbox}{\hlbox}\begin{minipage}[b]{.99\linewidth}}%
...
\begin{table}[p]
  ...
  \begin{small}
    \begin{tabular}{lccc}
      \toprule
      Package  & Any \code{gsl} header & \code{gsl\_vector.h} & \code{gsl\_matrix.h}  \\
      \midrule
      abn		 &    $\star$  &       $\star$        & $\star$     \\    
      BayesLogit	 &    $\star$  &                      &             \\ 
  ...
  \begin{flushleft}
    ...
    \code{grep} searching (recursively) for inclusion of any GSL header, or
    the vector and matrix headers specifically, within the \code{src/} or
    \code{inst/include/} directories of expanded source code archives of
...
\section[Motivation: fastLm]{Motivation: \code{fastLm}}
...
\code{lm()} which provides a model fit as well as a number of diagnostic
measure, either directly or via the \code{summary()} method for linear
model fits. The \code{lm.fit()} function provides a faster alternative
...
\code{fastLm} routine shown here provides such an implementation as part of
...
vector. Both use the standard numeric type \code{double} as discussed
...
\code{double} and \code{int}. For these latter two, we offer a shorthand
definition via a \code{typedef} which allows a shorter non-template use.
...
\code{free()} to return temporary memory allocation to the operating
...
when the object goes out of scope. Explicit \code{free()} calls are still
...
use of \code{const \&} in the interface.  This instructs the compiler that
...
\citep{CRAN:RcppArmadillo,Eddelbuettel+Sanderson:2014:RcppArmadillo}
implements a matching \code{fastLm} function using the Armadillo library by
\cite{Sanderson:2010:Armadillo}, and can do so with even more compact code due to
\proglang{C++} features. Moreover, \pkg{RcppEigen}
\citep{CRAN:RcppEigen,JSS:RcppEigen} provides a \code{fastLm} implementation with a
...
\subsection[GSL vector]{\pkg{GSL} vectors}
...
\subsection[RcppGSL::vector]{\code{RcppGSL::vector}}

\pkg{RcppGSL} defines the template \code{RcppGSL::vector<T>} to manipulate
...
The class \code{RcppGSL::vector<double>} is a smart pointer which can be deployed
...
managed release of memory either via \code{free()} or when going out of
scope), the \code{RcppGSL::vector} template faciliates interchange of
\pkg{GSL} vectors with \pkg{Rcpp} objects, and hence \pkg{R} objects. The
following example defines a \code{.Call} compatible function called
\verb|sum_gsl_vector_int| that operates on a \verb|gsl_vector_int| through
the \code{RcppGSL::vector<int>} template specialization:
...
Here we no longer need to call \code{free()} explicitly as the \code{vec}
allocation is returned automatically at the end of the function body when the
variable goes out of scope.

Once the function has created via \code{sourceCpp()} or
\code{cppFunction()} from \textsl{Rcpp Attributes} (see
...
\begin{table}[htb]
  ...
  \begin{small}
    \begin{tabular}{ll}
      ...
      GSL vector & RcppGSL \\
      \midrule
      \code{gsl\_vector} &            \code{RcppGSL::vector<double>} \\
                           &            \code{RcppGSL::Vector} \\
      \code{gsl\_vector\_int} & 	\code{RcppGSL::vector<int>} \\
                                &       \code{RcppGSL::IntVector} \\
      \code{gsl\_vector\_float} & 	\code{RcppGSL::vector<float>} \\
      \code{gsl\_vector\_long} & 	\code{RcppGSL::vector<long>} \\
      \code{gsl\_vector\_char} & 	\code{RcppGSL::vector<char>} \\
      \code{gsl\_vector\_complex} & 	\code{RcppGSL::vector<gsl\_complex>} \\
      \code{gsl\_vector\_complex\_float} & \code{RcppGSL::vector<gsl\_complex\_float>} \\
      \code{gsl\_vector\_complex\_long\_double} & 	\code{RcppGSL::vector<gsl\_complex\_long\_double>} \\
      \code{gsl\_vector\_long\_double} & 	\code{RcppGSL::vector<long double>} \\
      \code{gsl\_vector\_short} & 	\code{RcppGSL::vector<short>} \\
      \code{gsl\_vector\_uchar} & 	\code{RcppGSL::vector<unsigned char>} \\
      \code{gsl\_vector\_uint} & 	\code{RcppGSL::vector<unsigned int>} \\
      \code{gsl\_vector\_ushort} & 	\code{RcppGSL::vector<insigned short>} \\
      \code{gsl\_vector\_ulong} & 	\code{RcppGSL::vector<unsigned long>} \\
      \bottomrule
    \end{tabular}
  \end{small}
...
\code{double} and \code{int} vectors.  First, \code{RcppGSL::Vector} is a
short-hand for the \code{RcppGSL::vector<double>} template
instantiation. Second, \code{RcppGSL::IntVector} does the same for
...
type. \pkg{RcppGSL} defines the template class \code{RcppGSL::vector\_view}
...
\code{RcppGSL::vector\_view} can be converted implicitly to their
...
\begin{table}[htb]
  ...
  \begin{small}
    \begin{tabular}{ll}
      ...
      \midrule
      \code{gsl\_vector\_view}        & \code{RcppGSL::vector\_view<double>} \\
                                        & \code{RcppGSL::VectorView} \\
      \code{gsl\_vector\_view\_int}   & \code{RcppGSL::vector\_view<int>} \\
                                        & \code{RcppGSL::IntVectorView} \\
      \code{gsl\_vector\_view\_float} & 	\code{RcppGSL::vector\_view<float>} \\
      \code{gsl\_vector\_view\_long} & 	\code{RcppGSL::vector\_view<long>} \\
      \code{gsl\_vector\_view\_char} & 	\code{RcppGSL::vector\_view<char>} \\
      \code{gsl\_vector\_view\_complex} & 	\code{RcppGSL::vector\_view<gsl\_complex>} \\
      \code{gsl\_vector\_view\_complex\_float} & \code{RcppGSL::vector\_view<gsl\_complex\_float>} \\
      \code{gsl\_vector\_view\_complex\_long\_double} & 	\code{RcppGSL::vector\_view<gsl\_complex\_long\_double>} \\
      \code{gsl\_vector\_view\_long\_double} & 	\code{RcppGSL::vector\_view<long double>} \\
      \code{gsl\_vector\_view\_short} & 	\code{RcppGSL::vector\_view<short>} \\
      \code{gsl\_vector\_view\_uchar} & 	\code{RcppGSL::vector\_view<unsigned char>} \\
      \code{gsl\_vector\_view\_uint} & 	\code{RcppGSL::vector\_view<unsigned int>} \\
      \code{gsl\_vector\_view\_ushort} & 	\code{RcppGSL::vector\_view<insigned short>} \\
      \code{gsl\_vector\_view\_ulong} & 	\code{RcppGSL::vector\_view<unsigned long>} \\
      \bottomrule
    \end{tabular}
  \end{small}
...
before, \code{double} and \code{int} types can be accessed via the 
\code{typedef} variants \code{RcppGSL::VectorView} and
\code{RcppGSL::IntVectorView}, respectively.  

Lastly, in order to support \code{const \&} behaviour, all
\code{gsl\_vector\_XXX\_const\_view} variants are also supported (where
\code{XXX} stands for any of the atomistic \proglang{C} and \proglang{C++}
data types).

\section{Matrices}

The \pkg{GSL} also defines a set of matrix data types : \code{gsl\_matrix},
\code{gsl\_matrix\_int} etc ... for which \pkg{RcppGSL} defines a corresponding
convenience \proglang{C++} wrapper generated by the \code{RcppGSL::matrix}
template.

\subsection{Creating matrices}

The \code{RcppGSL::matrix} template exposes three constructors.
...
\code{RcppGSL::matrix} defines an implicit conversion to a pointer to the
associated \pkg{GSL} matrix type, as well as dereferencing operators. This
makes the class \code{RcppGSL::matrix} look and feel like a pointer to a
\pkg{GSL} matrix type.
...
\code{gsl\_matrix\_get}, \code{gsl\_matrix\_int\_get}, ... and
\code{gsl\_matrix\_set}, \code{gsl\_matrix\_int\_set}, ...
...
The \code{RcppGSL::matrix} type also defines the following member functions:
\begin{quote}
  \begin{itemize}
  \item[\code{nrow}] extracts the number of rows
  \item[\code{ncol}] extract the number of columns
  \item[\code{size}] extracts the number of elements
  \item[\code{free}] releases the memory (also called via destructor)
  \end{itemize}
\end{quote}
...
first by using \code{gsl\_set\_error\_handler\_off()}, and then detect
error conditions by checking whether the result is \code{NAN} or not.
...
\code{gslSetErrorHandlerOff()} and \code{gslResetErrorHandler()} which
...
\code{gslSetErrorHandlerOff()} when being attached, ensuring that the
\pkg{GSL} error handler is turned off by default. 


\section[Using RcppGSL in your package]{Using \pkg{RcppGSL} in your package}

The \pkg{RcppGSL} package contains a complete example package providing a single
function \code{colNorm} which computes a norm for each column of a
...
\subsection[The configure script]{The \code{configure} script}

\subsubsection[Using autoconf]{Using \code{autoconf}}
...
done easily from a \code{configure} source script which \code{autoconf}
generates from a \code{configure.in} source file such as the following:
...
A source file such as this \code{configure.in} gets converted into a script
\code{configure} by invoking the \code{autoconf} program.

We note that many other libraries use a similar (but somewhat newer and
by-now fairly standard) scripting frontend called \code{pkg-config} which
be deployed in a very similar by other packages. Calls such as the following
two can be used from \code{configure} in a very similar manner:
...
where \code{libpng} (for the png image format) is used just for illustration.

\subsubsection{Using functions provided by RcppGSL}

\pkg{RcppGSL} provides R functions (in the file \code{R/inline.R}) that
...
the \pkg{RcppZiggurat} \citep{CRAN:RcppZiggurat} package for an example.
...
supplied by \code{configure} during build-time:
...
The variables surrounded by \@ will be filled by \code{configure} during
package build-time. As discussed above, this can either rely on
\code{autoconf} or a possibly-simpler \code{Rscript}.

\subsection[The R directory]{The \proglang{R} directory}

The \proglang{R} source is very simply: it contains a single file created by
the \code{Rcpp::compileAttributes()} function implementing the wrapper to
the \code{colNorm()} function.

\section[Using RcppGSL with inline]{Using \pkg{RcppGSL} with \pkg{inline}}
...
The \code{RcppGSL} inline plugin supports creation of a package skeleton
...
\section[Using RcppGSL with Rcpp Attributes]{Using \pkg{RcppGSL} with \textsl{Rcpp Attributes}}
...
example used via \code{sourceCpp()}. We stress that usage is
\code{sourceCpp()} is meant for interactive work at the R command-prompt,
...
call \code{sourceCpp()} to have the wrapper code added, and all of the
...
The function \code{cppFunction()} is also available to convert a simple
character string argument containing a valid C++ function into a eponymous R
function. And like \code{sourceCpp()}, it can also use plugins. See the
