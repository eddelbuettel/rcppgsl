...
Support for \proglang{C} programming with the \pkg{GSL} is readily available: the
\pkg{GSL} itself is written in \proglang{C}
...
implementations.\footnote{Several \proglang{C++} wrappers for the \pkg{GSL} have
...
The \pkg{GSL} is somewhat unique among numerical libraries. Its combination of
broad coverage of scientific topics, serious implementation effort and the
use of the well-known GNU General Public License (GPL) have lead to a fairly
wide usage of the library.  As a concrete example, we can consider the the
CRAN repository network for the \proglang{R} language and environment
\citep{R:Main}.  As of July 2015, CRAN contains over a three dozen packages
interfacing the \pkg{GSL}; of these more than half interface the vector or
matrix classes (see Appendix A for details). This is a clear
indication that the \pkg{GSL} is popular among programmers using either the
\proglang{C} or \proglang{C++} language for solving problems applied science.
...
tries to offer access to \pkg{GSL} functions, in particular via the vector and
...
\texttt{lm()} which can provide a model fit as well as a number of diagnostic
...
The function interface defines two \textsl{RcppGSL} variables: a matrix and a
vector and matrix. These both use the standard numeric type \texttt{double}
as discussed below.  The \pkg{GSL} supports other types ranging from lower
precision floating point to signed and unsigned integers as well as complex
numbers, and the vector matrix classes are templated. Here we use a shorthand
definition via a \texttt{typdef}.  Having extracted the row and column
dimentions, we then reserve another vector and matrix to hold the resulting
coefficient estimates as well as the estimate of the covariance matrix. Next,
we allocate workspace using a \pkg{GSL} routine, fit the linear model and free the
just-allocated workspace.  The next step involves extracting the diagonal
element from the covariance matrix, and taking the square root (using a
vectorised function from \textsl{Rcpp}). Finally we create a named list with
the return values.

In earlier version of the \textsl{RcppGSL} package, we also explicitly called
...
accidentally be released more than once.
...
Note that we explicitly free the allocated memory at the end. With
...
We also define several convenient shortcuts for the very common case of
...
integer-valued vectors. These are provided primarily because \texttt{double}
and \texttt{int} are so common. Other types still require explicit templates.
...
List test_gsl_vector_view() {
    ...
    RcppGSL::vector_view<double> v_even =
        gsl_vector_subvector_with_stride(v,0,2,n/2);
    RcppGSL::vector_view<double> v_odd  =
        gsl_vector_subvector_with_stride(v,1,2,n/2);

    List res = List::create(Named("even") = v_even,
                            Named("odd" ) = v_odd);
    return res;
}
@
...
And as before, \texttt{double} and \texttt{int} types can be used by
\texttt{RcppGSL::VectorView} and \texttt{RcppGSL::IntVectorView}, respectively.
...
the prior (default) value.

In addition, the package now also calls \texttt{gslSetErrorHandlerOff()} when
being attached, ensuring that the \pkg{GSL} error handler is turned off by default.
...
link options, taking advantage of the installed \pkg{RcppGSL} package.
...
//
// [[Rcpp::export]]
Rcpp::NumericVector colNorm(RcppGSL::Matrix M) {
    int k = M.ncol();
    Rcpp::NumericVector n(k);           // to store results

    for (int j = 0; j < k; j++) {
        RcppGSL::VectorView colview = gsl_matrix_column (M, j);
        n[j] = gsl_blas_dnrm2(colview);
    }
    return n;                           // return vector
}
...
%\section{GSL Usage at CRAN}

% \pkg{copula}, \pkg{dynamo}, \pkg{gsl}, \pkg{gstat}, \pkg{magnets},
% \pkg{mvabund}, \pkg{QRMlib}, \pkg{RBrownie}, \pkg{RDieHarder}, \pkg{RHmm},
% \pkg{segclust}, \pkg{surveillance}, and \pkg{topicmodels}.  
...
\begin{table}[h]
  ...
  \begin{small}
    \begin{tabular}{lccc}
      \toprule
      package            &    gsl      &         gsl\_vector   & gsl\_matrix  \\
      \midrule
      abn		 &    $\star$  &       $\star$        & $\star$     \\    
      BayesLogit	 &    $\star$  &                      &             \\ 
    ...
    \end{tabular}
  \end{small}
  \caption{CRAN Package Usage of \pkg{GSL} ouright, for vectors and for matrices.}
  \label{tab:mappingVectors}
\end{table}
