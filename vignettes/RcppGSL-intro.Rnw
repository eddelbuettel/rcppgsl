...
\abstract{
  ...
  \citep{JSS:Rcpp,CRAN:Rcpp,Eddelbuettel:2013:Rcpp} which is
...
Support for \proglang{C} programming with the GSL is readily available: the
GSL itself is written in \proglang{C}
...
\citep{R:Main}.  CRAN contains over a dozen packages interfacing the GSL:
...
indication that the GSL is popular among programmers using either the
...
as discussed below.  The GSL supports other types ranging from lower
...
we allocate workspace using a GSL routine, fit the linear model and free the
...
being attached, ensuring that the GSL error handler is turned off by default.
