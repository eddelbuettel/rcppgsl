...
Support for \proglang{C} programming with the \pkg{GSL} is readily available as the
\pkg{GSL} itself is written in \proglang{C}, and provides a
\proglang{C}-language Application Programming Interface (API). 
Access from \proglang{C++} is also possible, albeit not at the
...
License (GPL) have lead to fairly wide usage of the library.  As a concrete
