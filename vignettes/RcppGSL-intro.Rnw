...
When input values for \pkg{GSL} functions are invalid,
the default error handler will abort the program after printing error
message, which will lead \proglang{R} to an abortion error.
To avoid this, we need to close it first by using \texttt{gsl\_set\_error\_handler\_off()}
and then detect the result is NAN or not.
