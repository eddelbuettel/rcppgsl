...
definition via a \texttt{typedef}.  Having extracted the row and column
...
vectorised function from \pkg{Rcpp}). Finally we create a named list with
the return values.
...
And as before, \texttt{double} and \texttt{int} types can be accessed via the
...
\begin{table}[h]
  ...
  \begin{flushleft}
  \footnotesize Data gathered by use of \texttt{grep} through expanded source
  code archives of the CRAN network. Access to such a code archive via WU
