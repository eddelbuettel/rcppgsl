...
estimate. Finally we create a named list with the return values. 
...
system. This step had to be done as the underlying objects are managed
as \proglang{C} objects. They conforming to the \pkg{GSL} interface and hence
without any automatic memory management. But as we actually provide a
\proglang{C++} data structure for matrix and vector objects, we can managed
them in the \proglang{C++} style and free the memory when the object goes out
of scope. Explicit \texttt{free()} calls are still permitted as we keep track
of the object status so that no memory is accidentally released twice.
...
Note that we explicitly free the allocated memory at the end. In \proglang{C}
style programming this is the responsibility of the programmer.
...
handler will abort the program after printing an error message, which will
lead \proglang{R} to an abortion error.  To avoid this, we need to close it
...
// feed GSL function with some invalid values
...
but not the recommended practice in a package.
