...
first by using \texttt{gsl\_set\_error\_handler\_off()} and then detect the
result is \texttt{NAN} or not.
...
See \url{http://thread.gmane.org/gmane.comp.lang.r.rcpp/7905} for a longer discussion.
...
can allow to turn off the error handler (as discussed above), and to reset to
...
example used via \texttt{sourceCpp()} --- which is not the recommended
practice in a package, but useful at the command-line.
